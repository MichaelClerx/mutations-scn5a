...
class WekaInput(base.Task):
    def __init__(self):
        super(WekaInput, self).__init__('weka_input')
        self._set_data_subdir('weka')
    ...
    def create_discretized(self, connection, filename, dva=True, moments=False,
        ...
        """
        Creates a WEKA file with properties of mutations and a discretized
        dva or dvi output.
        ...
        ``filename``
            The path/filename of the Weka file to create.
        ``dva``
        ...
        # Set up ARFF file
        a = arff.ARFFFile('mutations')
        a.set_comment('Mutation data for machine learning.')

        # Mutation properties
        a.add_numeric_field('index')
        if acid_names:
            a.add_nominal_field('old', 'ARNDCQEGHILKMFPSTWYV')
            a.add_nominal_field('new', 'ARNDCQEGHILKMFPSTWYV')
        ...
        for x in prefixes:
            for y in self._acid_property_names():
                a.add_numeric_field(x + '_' + y)

        # Acid similarity scores
        a.add_numeric_field('gonnet')
        a.add_numeric_field('grantham')

        # Positional properties
        #a.add_numeric_field('dom')
        a.add_numeric_field('hse')
        a.add_nominal_field('regtype', self._region_types(connection))
        a.add_nominal_field('segtype', self._segment_types(connection))
        a.add_nominal_field('side', self._side_types(connection))
        a.add_numeric_field('dseg4')
        a.add_numeric_field('dseg45')
        a.add_numeric_field('dseg5')
        a.add_numeric_field('dseg56')
        a.add_numeric_field('dseg6')
        a.add_numeric_field('dsegment')
        a.add_numeric_field('dlink34')
        a.add_numeric_field('dcterm')

        # Physical locations
        if False:
            a.add_numeric_field('x')
            a.add_numeric_field('y')
            a.add_numeric_field('z')
            a.add_numeric_field('r')
            a.add_numeric_field('t')
            a.add_numeric_field('tmod')

        # Output
        if dva:
            print('Using ACTIVATION, discretized outputs')
            a.add_nominal_field('dva_disc', discretization_names)
        else:
            print('Using INACTIVATION, discretized outputs')
            a.add_nominal_field('dvi_disc', discretization_names)
    ...
    def create_class(self, connection, filename, output='act', moments=False,
        ...
        """
        Creates a WEKA file with properties of mutations and a single
        ...
        a.add_nominal_field(output, [True, False])
    ...
    def _run(self):
        ...
        with base.connect() as connection:
            ...
            self.create_class(connection, filename, output='changed',
                deltas_only=True)
